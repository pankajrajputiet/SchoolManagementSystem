...
const create = asyncHandler(async (req, res) => {
  ...
  const response = new ApiResponse(201, 'Notification created', notification);
  res.status(response.statusCode).json(response);
});

const getAll = asyncHandler(async (req, res) => {
  const result = await notificationService.getNotifications(req.query, req.user);
  const response = new ApiResponse(200, 'Notifications fetched', result);
  res.status(response.statusCode).json(response);
});
...
const markAsRead = asyncHandler(async (req, res) => {
  ...
  const response = new ApiResponse(200, 'Notification marked as read', notification);
  res.status(response.statusCode).json(response);
});
